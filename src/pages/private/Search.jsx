...
import { searchMovie } from "../../services/playlistServices";
import { loaderActions } from "../../store/loaderSlice";

const Search = () => {
  const movieName = useSelector((store) => store.movieNameState);
  const loader = useSelector((store) => store.loader);
  const [movieData, setMovieData] = useState(null);
  const openModal = useSelector((store) => store.playlistModel);
  const dispatch = useDispatch();
  const handleClick = async () => {
    dispatch(playlistModelActions.toggleTrue());
  };
  ...
  useEffect(() => {
    if (movieName?.payload && movieName.payload !== "") {
      fetchMovie(movieName.payload);
    }
    return () => {
  ...
  return (
    <>
      ...
      <div className="px-4 py-2">
        {movieName && movieData && !loader && (
          <div
            className={`flex flex-col items-center gap-3 ${
              openModal ? "opacity-10" : ""
            }`}
          >
            <MovieTemplate movieData={movieData} />
            <button
              onClick={handleClick}
              className="bg-green-400 hover:bg-green-300 px-3 py-2 rounded-md"
            >
              Add to playlist
            </button>
          </div>
        )}
        {movieName && movieData && openModal && (
          <PlaylistModal movieName={movieData.Title} />
        )}
        {(!movieName || !movieData) && !loader && (
